...
JavaVM *m_jvm=NULL;  // Java Virtual Machine Object

jobject m_jobj=NULL; // Java Class Object
...
void changeBGM(const char* fname,int loop)
{
	...
	else if(endWith(fname,".wav"))
	{
		...
		if(wave!=NULL) {
			//printf("loop %d\n",loop);
			//loops: Indicates whether the audio should be played repeatedly. 0 means no repeats, -1 means infinite loop, and other positive numbers indicate the number of repeats.
...
{
	// Initialize the JavaVM object, globally unique
	if(m_jvm==NULL)
	{
	...
	//There is no need to release cls returned by GetObjectClass, because it is usually a global reference or a weak global reference.				
