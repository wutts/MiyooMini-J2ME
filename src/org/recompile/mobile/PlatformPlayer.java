...
/**
The so-called stop actually returns to the prefetched (buffered read) state, so there is no so-called stopped state. Generally, if the playback is completed, it will automatically return to the prefetched (buffered read) state
unrealized (not realized)----->realized (realized)----->prefetched (buffered read)----->started (playing state)
realize()            prefetch()                 start()

started (playing state)----->prefetched (buffered read) realized (realized)----->unrealized (not realized)
stop()                  deallocate() releases resources

The above 4 states can be returned to the closed state at any time using close()
**/
	...
	private class midiPlayer extends audioplayer
	{
		...
		public midiPlayer(InputStream stream, String type)
		{
			try
			{
				...
				// System.out.println("microseconds:"+mlength + " seconds:"+ts+" bgm:"+bgmFileName); 
				// midi.close();
		...
		{	
			...
			try{
				...
				//System.out.println(bgmFileName+"Start setting up playback");
				...
				// {
					...
					// TimerTask task = new TimerTask() {
						// @Override
						// public void run() {
							// System.out.println("Scheduled task executed once");
							
							// notifyListeners(PlayerListener.END_OF_MEDIA, 0);	
						// }
					// };
					//timer.schedule(task, js); // Execute the task after 5 seconds
				// }
			...
			//System.out.println("start:"+bgmFileName+" loop:"+loops);
		}
		...
		{	
			...
			try{
				...
				//System.out.println(bgmFileName+"stop");
			}
			catch(Exception e)
			{
				System.out.println(e.getMessage());
			...
			//System.out.println("closure:"+bgmFileName+" loop:"+loops);
		}
		public void deallocate()
		{
			// if(!isinit)
		...
		public void setLoopCount(int count)
		{
			...
			//System.out.println("Set playback count");
